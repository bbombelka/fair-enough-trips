...
(async function createLqip() {
  const { metadata } = await lqip(path.resolve(__dirname, `../../public/content/posts/${id}/main.webp`), { resize: 64 });
...
async function updatePostJson(id: string, base64Image: string) {
  const jsonPath = path.resolve(__dirname, `../../public/content/posts/${id}/post.json`);
