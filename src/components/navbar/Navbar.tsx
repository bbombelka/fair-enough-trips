    ...
    <nav className={styles.navbar}>
      <div className={styles["container-desktop"]}>
        ...
        {navbarLinks.map(({ name, href, nestedLinks }, i, links) => {
          ...
          const activeLinkClassname = pathname?.includes(href) ? styles["navbar-link-active"] : "";
          const linkClassname = clsx(styles["navbar-link"], lastLinkClassname, activeLinkClassname);

          return nestedLinks ? (
            <Tab key={i} name={name}>
              {nestedLinks.map(({ name, href }, i) => (
                <Link className={`${styles["navbar-link"]} ${pathname?.includes(href) ? styles["navbar-link-active"] : ""}`} key={i} href={href} name={name} />
              ))}
            </Tab>
          ) : (
